...
- ❌ Bad: _"Perform a review for PR #12 in the **GitHub** repository foo/bar"_ — this primes the orchestrator with the wrong platform when the remote is actually Azure DevOps or Bitbucket, leading to wasted `gh` calls and leaked tokens in logs.
- ✅ Good: _"Perform a review for PR #12 in repository `foo/bar` on branch `feature/x`. Detect the hosting platform from the git remote before doing anything else."_
